...
CREATE TABLE TREN (
    ...
    NOMBRE VARCHAR(50) CONSTRAINT TREN_NOMBRE_NN NOT NULL
                        CONSTRAINT TREN_NOMBRE_UK UNIQUE,
...
CREATE TABLE ESTACION (
    ...
    NOMBRE VARCHAR(100) CONSTRAINT ESTACION_NOMBRE_NN NOT NULL
                        CONSTRAINT ESTACION_NOMBRE_UK UNIQUE
);
...
CREATE TABLE RUTA_DETALLE (
    ...
    CONSTRAINT RUTA_DETALLE_ID_RUTA_FK FOREIGN KEY (ID_RUTA) REFERENCES RUTA(ID_RUTA) ON DELETE CASCADE,
    CONSTRAINT RUTA_DETALLE_ID_ESTACION_FK FOREIGN KEY (ID_ESTACION) REFERENCES ESTACION(ID_ESTACION) ON DELETE CASCADE
);
...
CREATE TABLE ASIGNACION_TREN (
    ...
    CONSTRAINT ASIGNACION_ID_TREN_FK FOREIGN KEY (ID_TREN) REFERENCES TREN(ID_TREN) ON DELETE CASCADE,
    CONSTRAINT ASIGNACION_ID_RUTA_FK FOREIGN KEY (ID_RUTA) REFERENCES RUTA(ID_RUTA) ON DELETE CASCADE,
    CONSTRAINT ASIGNACION_ID_HORARIO_FK FOREIGN KEY (ID_HORARIO) REFERENCES HORARIO(ID_HORARIO) ON DELETE CASCADE
);

CREATE TABLE INCIDENCIA (
    ID_INCIDENCIA INT CONSTRAINT INCIDENCIA_PK PRIMARY KEY,
    ID_ASIGNACION INT CONSTRAINT INCIDENCIA_ID_ASIGNACION_NN NOT NULL,
    TIPO VARCHAR(50) CONSTRAINT INCIDENCIA_TIPO_NN NOT NULL
					 CONSTRAINT INCIDENCIA_TIPO_CK CHECK (TIPO IN ('RETRASO', 'AVERIA', 'EMERGENCIA')),
    ...
    ESTADO VARCHAR2(12) DEFAULT 'RESUELTO' CONSTRAINT INCIDENCIA_ESTADO_NN NOT NULL
    CONSTRAINT INCIDENCIA_ESTADO_CK CHECK (ESTADO IN ('RESUELTO', 'NO RESUELTO'))
);
...
CREATE TABLE HISTORIAL (
    ...
    INFORMACION CLOB CONSTRAINT HISTORIAL_INFORMACION_NN NOT NULL,
    HORA_REAL VARCHAR2(20),
    ID_USUARIO INT CONSTRAINT HISTORIAL_ID_USUARIO_NN NOT NULL,
    ID_HORARIO INT,
    ID_RUTA INT,
    ID_ASIGNACION INT,
    ID_INCIDENCIA INT CONSTRAINT HISTORIAL_ID_INCIDENCIA_UK UNIQUE,
    CONSTRAINT HISTORIAL_ID_USUARIO_FK FOREIGN KEY (ID_USUARIO) REFERENCES USUARIO(ID_USUARIO),
    CONSTRAINT HISTORIAL_FKS_CK CHECK (
        (ID_HORARIO IS NOT NULL AND ID_RUTA IS NULL AND ID_ASIGNACION IS NULL AND HORA_REAL IS NULL AND ID_INCIDENCIA IS NULL) OR
        (ID_HORARIO IS NULL AND ID_RUTA IS NOT NULL AND ID_ASIGNACION IS NULL AND HORA_REAL IS NULL AND ID_INCIDENCIA IS NULL) OR
        (ID_HORARIO IS NULL AND ID_RUTA IS NULL AND ID_ASIGNACION IS NOT NULL AND HORA_REAL IS NULL AND ID_INCIDENCIA IS NULL) OR
        (ID_HORARIO IS NULL AND ID_RUTA IS NULL AND ID_ASIGNACION IS NOT NULL AND HORA_REAL IS NOT NULL AND ID_INCIDENCIA IS NULL) OR
        (ID_HORARIO IS NULL AND ID_RUTA IS NULL AND ID_ASIGNACION IS NOT NULL AND HORA_REAL IS NULL AND ID_INCIDENCIA IS NOT NULL)
    )
);
...
-- EL VALOR ORDEN EN LA TABLA RUTA_DETALLE SE ACTUALIZA DESPUES DE BORRAR UNA ESTACION
-- 1. Crear tabla temporal
CREATE GLOBAL TEMPORARY TABLE TMP_RUTA_ELIMINADA (
    ID_RUTA INT,
    ORDEN INT
) ON COMMIT DELETE ROWS;
/

-- 2. Trigger BEFORE DELETE en RUTA_DETALLE
CREATE OR REPLACE TRIGGER TRG_BEFORE_DELETE_RUTA_DETALLE
BEFORE DELETE ON RUTA_DETALLE
FOR EACH ROW
BEGIN
    INSERT INTO TMP_RUTA_ELIMINADA (ID_RUTA, ORDEN)
    VALUES (:OLD.ID_RUTA, :OLD.ORDEN);
END;
/

-- 3. Trigger AFTER DELETE en RUTA_DETALLE
CREATE OR REPLACE TRIGGER TRG_AFTER_DELETE_RUTA_DETALLE
AFTER DELETE ON RUTA_DETALLE
DECLARE
BEGIN
    FOR r IN (SELECT * FROM TMP_RUTA_ELIMINADA) LOOP
        UPDATE RUTA_DETALLE
        SET ORDEN = ORDEN - 1
...
VALUES (1, 1, 'RETRASO', 'Retraso por mantenimiento en vías', SYSDATE, 'RESUELTO');

INSERT INTO INCIDENCIA (ID_INCIDENCIA, ID_ASIGNACION, TIPO, DESCRIPCION, FECHA_HORA) 
...
INSERT INTO USUARIO (ID_USUARIO, NOMBRE, APELLIDO_PATERNO, APELLIDO_MATERNO, CONTRASENA)
VALUES (9999, 'ADMIN', 'ADMIN', 'ADMIN', 'ADMIN_CONTROL_TRENES_0000');
...
VALUES (2, SYSDATE, 2, 2, 'Duracion estimada 100');
...
-- Registro de 2 registros de incidencias
INSERT INTO HISTORIAL (ID_HISTORIAL, FECHA_REGISTRO, ID_ASIGNACION, ID_USUARIO, INFORMACION, ID_INCIDENCIA) 
VALUES (4, SYSDATE, 1, 1, 'Horario: 08:30:36 - 10:01:02; Orden: A - B - C ', 1);

INSERT INTO HISTORIAL (ID_HISTORIAL, FECHA_REGISTRO, ID_ASIGNACION, ID_USUARIO, INFORMACION, ID_INCIDENCIA) 
VALUES (5, SYSDATE, 2, 1, '08:30:36 - 10:01:02; Orden: A - B - C ', 2);
