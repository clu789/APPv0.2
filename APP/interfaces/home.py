from PyQt6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QLabel, QTableWidget, QTableWidgetItem,
                             QScrollArea, QScrollBar, QTableView, QPushButton, QFrame, QSplitter)
...
class InterfazHome(QWidget):
    ...
    def initUI(self):
        ...
        self.tabla_viajes.setHorizontalHeaderLabels([
            "ID Ruta", "Horario", "Estación Origen", "Estación Destino", "Unidad Asignada", "Estado"
        ])
        ...
        self.tabla_proximos.setColumnCount(6)
    ...
    def cargar_datos_viajes(self):
        query = """
            SELECT A.ID_RUTA, 
                ...
                T.ID_TREN,
                CASE 
                    WHEN H.HORA_SALIDA_PROGRAMADA <= SYSDATE AND H.HORA_LLEGADA_PROGRAMADA > SYSDATE THEN 'En movimiento'
                    WHEN H.HORA_LLEGADA_PROGRAMADA <= SYSDATE THEN 'Detenido en estación'
                    ELSE 'Programado'
                END AS ESTADO
